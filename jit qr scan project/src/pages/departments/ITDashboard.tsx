...
const getCategory = (c: Circular): string => {
  ...
  if (/(workshop|seminar|training)/.test(text)) return 'Workshop';
  if (/(event|fest|celebration|symposium)/.test(text)) return 'Events';
  if (/(holiday|leave|vacation)/.test(text)) return 'Holidays';
            ...
            {filteredCirculars.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="it-paper-title" style={{ textAlign: 'left', marginTop: 16 }}>{c.title}</h3>
