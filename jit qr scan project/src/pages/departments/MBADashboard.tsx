...
const MBADashboard: React.FC = () => {
  ...
  const allMbaCirculars = useMemo(() => {
    return getCircularsForDept('Master of Business Administration').filter((c) => c.status === 'active');
  }, [getCircularsForDept]);
        ...
        <div className="mba-modal-overlay" onClick={() => setSelectedNotice(null)}>
          <div className="mba-modal-paper" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {selectedNotice.posterImage && (
                <button onClick={() => downloadFile(selectedNotice.posterImage!, selectedNotice.title + '.png')} style={{ flex: 1, padding: '10px', background: 'white', color: 'var(--mba-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              {selectedNotice.pdfFile && (
                <>
                  ...
                  <button onClick={() => downloadFile(selectedNotice.pdfFile!, selectedNotice.pdfName ?? 'notice.pdf')} style={{ flex: 1, padding: '10px', background: 'white', color: 'var(--mba-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
