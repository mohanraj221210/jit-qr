...
const AIDSDashboard: React.FC = () => {
  ...
  const allAidCirculars = useMemo(() => {
    return getCircularsForDept('AI&DS').filter((c) => c.status === 'active');
  }, [getCircularsForDept]);
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {selectedNotice.pdfFile && (
                <>
                  ...
                  <button onClick={() => downloadPdf(selectedNotice.pdfFile!, selectedNotice.pdfName ?? 'notice.pdf')} style={{ flex: 1, padding: '10px', background: 'white', color: 'var(--ai-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
