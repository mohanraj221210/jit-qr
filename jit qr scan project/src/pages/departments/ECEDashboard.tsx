...
const getPriority = (c: Circular): 'urgent' | 'important' | 'normal' => {
  ...
  if (/(important|attention|exam|placement|symposium|visit)/.test(text)) return 'important';
  return 'normal';
};
    ...
    <div className="ec-wrapper">
      ...
      <div className="ec-signboard-wood">
        <div className="ec-signboard-inner">
          ...
          {/* Decorative ECE Icons Background */}
          <CircuitBoard className="ec-sign-bg-icon" size={32} style={{ top: 10, left: 100 }} />
          <Cpu className="ec-sign-bg-icon" size={24} style={{ top: 50, left: 70 }} />
          <Antenna className="ec-sign-bg-icon" size={28} style={{ top: 15, right: 90 }} />
          <Radio className="ec-sign-bg-icon" size={22} style={{ top: 55, right: 60 }} />
          <Microchip className="ec-sign-bg-icon" size={20} style={{ bottom: 10, left: 130 }} />
          <Wifi className="ec-sign-bg-icon" size={18} style={{ bottom: 10, right: 120 }} />
          <Satellite className="ec-sign-bg-icon" size={26} style={{ top: 25, left: 150 }} />
          ...
          <div className="ec-sign-meta">
            ...
            <div className="ec-meta-item">
              <FileText size={14} />
              <span>{allEcCirculars.length} Active Notices</span>
            </div>
            <div className="ec-meta-divider" />
            <div className="ec-meta-item">
      ...
      <div className="ec-controls">
        <div className="ec-search-row">
          ...
          <button className="ec-filter-btn">
        ...
        <div className="ec-filter-chips">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.name}
              className={`ec-chip ${selectedCategory === cat.name ? 'active' : ''}`}
      ...
      {/* ────────────── CORK BOARD ────────────── */}
      <main className="ec-cork-board">
        <div className="ec-board-glass" />
        {filteredCirculars.length === 0 ? (
          <>
            ...
            <div className="ec-bottom-strip ec-torn-top ec-torn-bottom">
              <div className="ec-push-pin pin-gray" style={{ top: 12, left: 16 }} />
              <div className="ec-push-pin pin-gray" style={{ top: 12, left: 'auto', right: 16 }} />
              <div className="ec-strip-left">
                <Bell size={24} color="#6B7280" />
                <div>
                  <h4 className="ec-strip-title">Get instant updates</h4>
                  <p className="ec-strip-subtitle">New notices will appear automatically.</p>
                </div>
              </div>
              <button className="ec-strip-refresh" onClick={handleRefresh}>
            ...
            {paginatedCirculars.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="ec-paper-title">{c.title}</h3>
      ...
      {/* ────────────── NOTICE MODAL ────────────── */}
      {selectedNotice && (
        <div className="ec-modal-overlay" onClick={() => setSelectedNotice(null)}>
          <div className="ec-modal-paper" onClick={(e) => e.stopPropagation()}>
            <button className="ec-modal-close" onClick={() => setSelectedNotice(null)}>
              <X size={20} />
            </button>
            <div style={{ marginBottom: 16, borderBottom: '1px dashed #E5E7EB', paddingBottom: 12 }}>
              <div style={{ fontSize: 12, color: 'var(--ec-primary)', fontWeight: 600, marginBottom: 8 }}>{getCategory(selectedNotice)}</div>
              <h2 style={{ margin: '0 0 8px', fontSize: 22, color: 'var(--ec-text-dark)' }}>{selectedNotice.title}</h2>
              <div style={{ fontSize: 13, color: 'var(--ec-text-muted)', display: 'flex', gap: 12 }}>
