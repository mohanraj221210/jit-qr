...
const CATEGORIES = [
  ...
  { name: 'Coding Contest', icon: <Terminal size={14} /> },
...
const getPriority = (c: Circular): 'urgent' | 'important' | 'normal' => {
  ...
  if (/(important|attention|exam|placement|hackathon|contest)/.test(text)) return 'important';
  return 'normal';
};

const getCategory = (c: Circular): string => {
  const text = `${c.title} ${c.description || ''} ${c.eventName || ''}`.toLowerCase();
  if (/(coding contest|leetcode|codechef|hacker rank)/.test(text)) return 'Coding Contest';
...
const CSEDashboard: React.FC = () => {
  const { getCircularsForDept } = useCirculars();

  // Load CSE active circulars
  const allCsCirculars = useMemo(() => {
    return getCircularsForDept('Computer Science and Engineering').filter((c) => c.status === 'active');
  }, [getCircularsForDept]);
  ...
  const filteredCirculars = useMemo(() => {
    return allCsCirculars.filter((c) => {
      const matchSearch =
  ...
  }, [allCsCirculars, searchQuery, selectedCategory]);
    ...
    <div className="cs-wrapper">
      ...
      <div className="cs-signboard-wood">
        <div className="cs-signboard-inner">
          ...
          {/* Decorative CS Icons Background */}
          <Code className="cs-sign-bg-icon" size={32} style={{ top: 10, left: 100 }} />
          <Terminal className="cs-sign-bg-icon" size={24} style={{ top: 50, left: 70 }} />
          <Database className="cs-sign-bg-icon" size={28} style={{ top: 15, right: 90 }} />
          <Shield className="cs-sign-bg-icon" size={22} style={{ top: 55, right: 60 }} />
          <Binary className="cs-sign-bg-icon" size={20} style={{ bottom: 10, left: 130 }} />
          <Cloud className="cs-sign-bg-icon" size={18} style={{ bottom: 10, right: 120 }} />
          <Laptop className="cs-sign-bg-icon" size={26} style={{ top: 25, left: 150 }} />
          <Network className="cs-sign-bg-icon" size={20} style={{ top: 25, right: 150 }} />
          <Monitor className="cs-sign-bg-icon" size={20} style={{ top: 75, left: 200 }} />
          <Globe className="cs-sign-bg-icon" size={16} style={{ top: 70, right: 190 }} />
          <Cpu className="cs-sign-bg-icon" size={18} style={{ bottom: 20, right: 220 }} />
          ...
          <div className="cs-sign-meta">
            ...
            <div className="cs-meta-item">
              <FileText size={14} />
              <span>{allCsCirculars.length} Active Notices</span>
            </div>
            <div className="cs-meta-divider" />
            <div className="cs-meta-item">
      ...
      <div className="cs-controls">
        ...
        <div className="cs-filter-chips">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.name}
              className={`cs-chip ${selectedCategory === cat.name ? 'active' : ''}`}
        ...
        {filteredCirculars.length === 0 ? (
          <>
            ...
            <div className="cs-bottom-strip cs-torn-top cs-torn-bottom">
              <div className="cs-push-pin pin-gray" style={{ top: 12, left: 16 }} />
              <div className="cs-push-pin pin-gray" style={{ top: 12, left: 'auto', right: 16 }} />
              <div className="cs-strip-left">
                <Bell size={24} color="#6B7280" />
                <div>
                  <h4 className="cs-strip-title">Get instant updates</h4>
                  <p className="cs-strip-subtitle">New notices will appear automatically.</p>
                </div>
              </div>
              <button className="cs-strip-refresh" onClick={handleRefresh}>
            ...
            {paginatedCirculars.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="cs-paper-title">{c.title}</h3>
      ...
      {/* ────────────── NOTICE MODAL ────────────── */}
      {selectedNotice && (
        <div className="cs-modal-overlay" onClick={() => setSelectedNotice(null)}>
          <div className="cs-modal-paper" onClick={(e) => e.stopPropagation()}>
            <button className="cs-modal-close" onClick={() => setSelectedNotice(null)}>
              <X size={20} />
            </button>
            <div style={{ marginBottom: 16, borderBottom: '1px dashed #E5E7EB', paddingBottom: 12 }}>
              <div style={{ fontSize: 12, color: 'var(--cs-primary)', fontWeight: 600, marginBottom: 8 }}>{getCategory(selectedNotice)}</div>
              <h2 style={{ margin: '0 0 8px', fontSize: 22, color: 'var(--cs-text-dark)' }}>{selectedNotice.title}</h2>
              <div style={{ fontSize: 13, color: 'var(--cs-text-muted)', display: 'flex', gap: 12 }}>
            ...
            <div style={{ fontSize: 15, lineHeight: 1.6, color: 'var(--cs-text-dark)', marginBottom: 24, whiteSpace: 'pre-wrap' }}>
              {selectedNotice.description}
            </div>

            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {selectedNotice.posterImage && (
                <button onClick={() => downloadFile(selectedNotice.posterImage!, selectedNotice.title + '.png')} style={{ flex: 1, padding: '10px', background: 'white', color: 'var(--cs-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  <Download size={16} /> Download Poster
                </button>
              )}
              {selectedNotice.pdfFile && (
                <>
                  <button onClick={() => openPdf(selectedNotice.pdfFile!)} style={{ flex: 1, padding: '10px', background: 'var(--cs-primary)', color: 'white', border: 'none', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    <FileText size={16} /> View PDF
                  </button>
                  <button onClick={() => downloadFile(selectedNotice.pdfFile!, selectedNotice.pdfName ?? 'notice.pdf')} style={{ flex: 1, padding: '10px', background: 'white', color: 'var(--cs-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    <Download size={16} /> Download PDF
                  </button>
                </>
              )}
              <button onClick={() => handleShare(selectedNotice)} style={{ padding: '10px', background: 'white', color: 'var(--cs-text-dark)', border: '1px solid #D1D5DB', borderRadius: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Share2 size={16} />
              </button>
            </div>
          </div>
